...
public class StoreDSLImpl implements StoreDSL {
    ...
    @Override
    public Page<StoreDTO> getStoreWithPage(Pageable pageable, Long memberId) {
        final List<StoreFileDTO> files =
                query.select(Projections.fields(StoreFileDTO.class,
                        storeFile.id,
                        storeFile.fileName,
        ...
        List<StoreDTO> stores = query.select(
                Projections.fields(StoreDTO.class,
                        store.id,
                        store.storeTitle,
                        store.storeContent,
        ...
        Long count = query.select(store.count())
                .from(store)
                .where(store.deleted.eq(false)
                        .and(store.member.id.eq(memberId)))
                .fetchOne();
